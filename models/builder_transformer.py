...
class PhaseProgressEncoder(nn.Module):
    ...
    def forward(
        self, phase: torch.Tensor, progress: torch.Tensor  # (B,) int  # (B,) float
    ) -> torch.Tensor:
        """
...
class BuilderTransformer(nn.Module):
    ...
    def predict_action(
        ...
        block_type = (
            block_logits.argmax(dim=-1)
            if not sample
            else torch.multinomial(
                F.softmax(block_logits / temperature, dim=-1), 1
            ).squeeze(-1)
        )
...
class BuilderTransformerSmall(BuilderTransformer):
    ...
    def __init__(self, max_size: int = 16, **kwargs):
        super().__init__(
            max_size=max_size, d_model=128, n_heads=4, n_layers=4, d_ff=512, **kwargs
        )
