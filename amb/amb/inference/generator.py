...
class StructureGenerator:
    ...
    def generate(
        ...
        if starting_state is not None:
            ...
            state = np.zeros((self.max_size, self.max_size, self.max_size), dtype=np.int32)
        ...
        for step in range(max_steps):
            ...
            phase_tensor = torch.tensor([current_phase], dtype=torch.long).to(self.device)
            progress_tensor = torch.tensor([progress], dtype=torch.float32).to(self.device)
    ...
    def _is_valid_placement(
        ...
        # Bounds check
        if not (0 <= x < self.max_size and 
                0 <= y < self.max_size and 
                0 <= z < self.max_size):
            return False
