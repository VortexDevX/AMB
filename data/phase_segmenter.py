...
class PhaseSegmenter:
    ...
    def segment(self, roles: np.ndarray) -> np.ndarray:
        ...
        foundation_mask = (
            (np.arange(H)[None, :, None] == min_y) | (roles == Role.FLOOR)
        ) & non_air_mask
        phases[foundation_mask] = Phase.FOUNDATION

        # ROOF: y == max_y or role == ROOF
        roof_mask = (
            ((np.arange(H)[None, :, None] == max_y) | (roles == Role.ROOF))
            & non_air_mask
            & ~foundation_mask
        )
        ...
        wall_mask = (roles == Role.WALL) & non_air_mask & ~foundation_mask & ~roof_mask
